  ...
  const validateAndInsertData = async () => {
    ...
    for (const row of data) {
      ...
      if (selectedTable === "Registros") {
        ...
        const existingmaterial = materialsToInsert.find(record => record.material_code === material_code);
        const existindatamaterial = existingMaterials.find(record => record.material_code === material_code);
        if(existingmaterial){
          cont1 = cont1 + 1;
          completedTasks += 1;
          continue;
        }
        if(existindatamaterial){
          cont2 = cont2 + 1;
          completedTasks += 1;
          continue;
        }
  ...
  return (
    <>
      ...
      <ModalContent bgColor="gray.200">
        ...
        <Input value={Datafilter} onClick={() => setDatafilter()} onBlur={(e) => SearchFilter(e)} mt={2} width="30%" border="1px" borderColor="gray.500" height="80%" isDisabled={!showDatabaseData} placeholder={selectedTable === "Registros" ? "Orden de Compra" : selectedTable === "Materiales" ? "Codigo de Material" : "Nombre Proveedor"} ></Input>
